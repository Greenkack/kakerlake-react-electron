...
export function parseEuroLoose(input) {
    if (input == null)
        return null;
 ...
 */
export async function parseXlsxToMatrix(file) {
    ...
    if (!sheetName)
        throw new Error("Leere Arbeitsmappe.");
    const ws = wb.Sheets[sheetName];
    // In 2D-Array umwandeln
    const data = XLSX.utils.sheet_to_json(ws, { header: 1 });
    if (!data.length)
        throw new Error("Leeres Blatt.");
    // Kopfzeile
    ...
    for (let r = 1; r < data.length; r++) {
        ...
        if (!Number.isFinite(modules))
            continue; // Skip ungültig/leere Zeilen
 ...
 */
export function lookupPriceEuro(table, modules, storageModel) {
    ...
    if (price == null) {
        throw new Error(`Kein Preis für Speichermodell "${storageModel}" in Zeile (Module=${modules}).`);
    }
    return price;
}
/** Komfort: "Kein Speicher" als Fallback-Key ermitteln (falls vorhanden) */
export function getNoStorageKey(table) {
