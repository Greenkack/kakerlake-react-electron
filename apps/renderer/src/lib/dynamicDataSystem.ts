...
export interface DynamicDataItem {
  ...
  dataType: 'string' | 'number' | 'boolean' | 'array' | 'object' | 'calculation' | 'chart' | 'image';
...
export class DynamicDataManager {
  private data: Map<string, DynamicDataItem> = new Map();
  private categories: Map<string, DynamicDataCategory> = new Map();
  ...
  setData(key: string, value: any, options: {
    category: string;
    label: string;
    description?: string;
    dataType: DynamicDataItem['dataType'];
    priority?: DynamicDataItem['priority'];
    includedInPdf?: boolean;
  }) {
    const pdfBytes = this.calculateBytes(value, options.dataType);
    
    const item: DynamicDataItem = {
      key,
      value,
  ...
  private calculateBytes(value: any, dataType: DynamicDataItem['dataType']): number {
    switch (dataType) {
      case 'string':
  ...
  private updateCategory(categoryKey: string, item: DynamicDataItem) {
  ...
  // Daten abrufen
  getData(key: string): DynamicDataItem | undefined {
    return this.data.get(key);
  }

  // Alle Daten einer Kategorie
  getCategoryData(categoryKey: string): DynamicDataCategory | undefined {
    return this.categories.get(categoryKey);
  }

  // Alle Kategorien
  getAllCategories(): DynamicDataCategory[] {
    return Array.from(this.categories.values());
  }

  // PDF-Einstellungen setzen
  setPdfInclusion(key: string, included: boolean) {
...
export const useDynamicData = () => {
  return {
    setData: (key: string, value: any, options: any) => dynamicData.setData(key, value, options),
    getData: (key: string) => dynamicData.getData(key),
    getAllCategories: () => dynamicData.getAllCategories(),
    setPdfInclusion: (key: string, included: boolean) => dynamicData.setPdfInclusion(key, included),
