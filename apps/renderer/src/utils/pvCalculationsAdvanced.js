...
// GRUNDBERECHNUNGEN
// =============================================================================
export const calculateAnnualEnergyYield = (pvPowerKwp, specificYieldKwhPerKwp = 950) => {
    return safe(pvPowerKwp) * safe(specificYieldKwhPerKwp, 950);
};
export const calculateSelfConsumptionQuote = (selfConsumedKwh, totalGenerationKwh) => {
...
export const calculateAutarkyDegree = (selfConsumedKwh, totalConsumptionKwh) => {
    const selfConsumed = safe(selfConsumedKwh);
    const totalCons = safe(totalConsumptionKwh);
    return totalCons === 0 ? 0 : Math.min(100, (selfConsumed / totalCons) * 100);
};
export const calculateSpecificYield = (annualYieldKwh, pvPowerKwp) => {
    return safeDivide(safe(annualYieldKwh), safe(pvPowerKwp), 0);
};
export const calculatePerformanceRatio = (actualYieldKwh, globalRadiationKwhPerM2, pvAreaM2) => {
...
// =============================================================================
export const calculatePaybackPeriod = (investmentCosts, annualSavings) => {
    return safeDivide(safe(investmentCosts), safe(annualSavings), Infinity);
};
export const calculateAnnualCostSavings = (selfConsumedKwh, electricityPrice) => {
    return safe(selfConsumedKwh) * safe(electricityPrice, 0.30);
};
export const calculateFeedInTariffRevenue = (feedInKwh, feedInRateEur) => {
    return safe(feedInKwh) * safe(feedInRateEur, 0.08);
};
export const calculateNetPresentValue = (investment, annualSavings, years = LIFESPAN_YEARS, discountRate = DISCOUNT_RATE) => {
...
export const calculateIRR = (investment, annualSavings, years = LIFESPAN_YEARS) => {
    const inv = safe(investment);
    const savings = safe(annualSavings);
    const period = Math.max(1, Math.floor(safe(years, LIFESPAN_YEARS)));
    if (inv <= 0 || savings <= 0)
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        if (rate < -0.99 || rate > 10)
            break; // Plausibilitätsgrenzen
    }
    return Math.max(0, rate * 100);
};
export const calculateTotalROI = (investment, annualSavings, years = LIFESPAN_YEARS) => {
    const inv = safe(investment);
    const savings = safe(annualSavings);
    const period = Math.max(1, Math.floor(safe(years, LIFESPAN_YEARS)));
    if (inv <= 0)
...
// SPEICHERBERECHNUNGEN
// =============================================================================
export const calculateStorageCoverageDegree = (storedSelfConsumptionKwh, totalSelfConsumptionKwh) => {
    const stored = safe(storedSelfConsumptionKwh);
    const total = safe(totalSelfConsumptionKwh);
    return safeDivide(stored * 100, total, 0);
};
export const calculateOptimalStorageSize = (dailyConsumptionKwh, lossesPercent = 10) => {
    const dailyCons = safe(dailyConsumptionKwh);
    const losses = clamp(safe(lossesPercent, 10), 0, 50);
    return dailyCons * (1 - losses / 100);
};
export const calculateEmergencyPowerCapacity = (storageKwh, usableCapacityPercent = 80) => {
...
// =============================================================================
export const calculateCo2Savings = (annualYieldKwh, co2FactorKgPerKwh = CO2_EMISSIONS_GRID_KWH) => {
    return safe(annualYieldKwh) * safe(co2FactorKgPerKwh, CO2_EMISSIONS_GRID_KWH);
};
export const calculateCo2PaybackTime = (pvSizeKwp, annualProductionKwh) => {
...
export const calculateDetailedCo2Analysis = (annualProductionKwh, systemKwp, years = LIFESPAN_YEARS) => {
...
// TECHNISCHE ANALYSEN
// =============================================================================
export const calculatePvModuleEfficiency = (modulePowerWp, moduleAreaM2) => {
...
export const calculateDcAcOversizingFactor = (pvPowerKwp, inverterPowerKw) => {
    return safeDivide(safe(pvPowerKwp), safe(inverterPowerKw), Infinity);
};
// =============================================================================
// FINANZIERUNGSBERECHNUNGEN
// =============================================================================
export const calculateAnnuity = (principal, annualInterestRate, durationYears) => {
...
export const calculateLeasingCosts = (totalInvestment, leasingFactor, durationMonths, residualValuePercent = 1) => {
...
// =============================================================================
export const runMonteCarloSimulation = (baseInvestment, baseAnnualBenefit, nSimulations = 1000, confidenceLevel = 95) => {
...
// =============================================================================
export const generateOptimizationSuggestions = (systemKwp, annualProduction) => {
    const suggestions = [
        {
...
// =============================================================================
export const calculateComprehensivePvAnalysis = (input) => {
    const warnings = [];
    try {
        ...
        const selfConsumption = safe(input.self_consumption_kwh, Math.min(annualProduction * 0.3, annualConsumption));
        ...
        if (pvPower <= 0)
            warnings.push("PV-Leistung nicht angegeben");
        if (annualProduction <= 0)
            warnings.push("Jahresertrag nicht plausibel");
        if (investmentCosts <= 0)
            warnings.push("Investitionskosten nicht angegeben");
        // Grundberechnungen
        ...
        extendedResult.detailed_co2_analysis = calculateDetailedCo2Analysis(annualProduction, pvPower);
        // Monte-Carlo-Simulation (nur bei plausiblen Daten)
        if (annualSavings > 0 && investmentCosts > 0) {
            try {
                extendedResult.monte_carlo_analysis = runMonteCarloSimulation(investmentCosts, annualSavings);
            }
            catch (e) {
        ...
        // Optimierungsvorschläge
        extendedResult.optimization_suggestions = generateOptimizationSuggestions(pvPower, annualProduction);
...
export const useAdvancedPvCalculations = (input) => {
    return useMemo(() => {
        if (!input)
            return null;
