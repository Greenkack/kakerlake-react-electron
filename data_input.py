...
def render_data_input(texts: Dict[str, str]) -> None: 
    ...
    with col1:
        if st.button("Photovoltaik", key="pv_only_mode_btn", use_container_width=True):
            st.session_state.demand_mode_selection = "Nur Photovoltaik"
            st.success("Nur Photovoltaik-Modus ausgewählt")
            st.rerun()
    
    with col2:
        if st.button("Wärmepumpe", key="wp_only_mode_btn", use_container_width=True):
            st.session_state.demand_mode_selection = "Nur Wärmepumpe"
            st.success("Nur Wärmepumpe-Modus ausgewählt")
            st.rerun()
    
    with col3:
        if st.button("Photovoltaik + Wärmepumpe", key="pv_wp_combined_mode_btn", use_container_width=True):
    ...
    if current_mode:
        ...
        st.info(f"**Aktueller Modus:** {mode_names.get(current_mode, current_mode)}")
    else:
    ...
    if current_mode:
        ...
        if current_mode in ['Nur Photovoltaik', 'Photovoltaik + Wärmepumpe']:
            mode_info.append("Konfigurieren Sie die Photovoltaik-Anlage in den Projektdetails unten")
        
        if current_mode in ['wp_only', 'pv_wp_combined']:
            mode_info.append("Konfigurieren Sie die Wärmepumpe in den entsprechenden Bereichen")
    ...
    if inputs['customer_data']['financing_requested']:
        ...
        with st.expander(" **Finanzierungsdetails zum Photovoltaik-Vorhaben**", expanded=False):
            ...
            st.info(f" Geschätzte Gesamtkosten der Photovoltaik-Anlage: {estimated_total_costs:,.2f} €")
